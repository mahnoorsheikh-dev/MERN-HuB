...
export default function Topics() {
  ...
  const filtered = topics.filter((topics) =>
    topics.title.toLowerCase().includes(search.toLowerCase())
  );
        ...
        {filtered.map((topics) => (
          ...
          >
            <div className="flex justify-between items-start mb-3">
              ...
              <span className={`text-xs border px-2 py-1 rounded-full ${levelColor[topics.level]}`}>
                {topics.level}
              </span>
            </div>
            <p className="text-[#A0A0A0] text-sm">{topics.description}</p>
          </Link>
        ))}
      </div>
